<script lang="ts">
  ...
  import type { PostModRunners } from "../../../routes/api/mod/runners/+server";
  ...
  const onsubmit = async () => {
    const res = await req<PostModRunners>("POST", "/api/mod/runners", currentState);
    if (res.ok) {
      if (res.data.created) {
        invalidate("mod:runners");
        onclose?.();
      }
    } else {
